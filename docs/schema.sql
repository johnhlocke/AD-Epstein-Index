...
CREATE TABLE wealth_profiles (
  ...
  -- Archival biographical summary (generated by src/generate_bios.py)
  bio_summary TEXT,
...
-- ============================================================
-- Fame Metrics (measure_fame.py output)
-- ============================================================

-- Internet-derived fame metrics per homeowner, based on
-- Ramirez & Hagen (2018) "The Quantitative Measure and Statistical
-- Distribution of Fame" (PLOS ONE).
CREATE TABLE fame_metrics (
  id BIGINT PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
  homeowner_name TEXT UNIQUE NOT NULL,
  group_label TEXT NOT NULL,  -- 'epstein' or 'baseline'
  feature_id BIGINT REFERENCES features(id),

  -- Wikipedia metrics (free, Phase 1)
  wikipedia_page TEXT,
  wikipedia_edit_count INTEGER,
  wikipedia_pageviews BIGINT,

  -- Google metrics via SerpAPI (Phase 2)
  google_search_results BIGINT,
  google_news_total BIGINT,
  google_news_visible INTEGER,

  -- NewsAPI metrics (Phase 3)
  newsapi_article_count INTEGER,

  -- NYT Article Search (Phase 4)
  nyt_article_count INTEGER,

  -- Wikipedia reference/citation count (Phase 5)
  wikipedia_reference_count INTEGER,

  -- Geometric mean fame score (computed at apply time)
  fame_score FLOAT,

  collected_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_fame_group ON fame_metrics(group_label);
CREATE INDEX idx_fame_name ON fame_metrics(homeowner_name);
