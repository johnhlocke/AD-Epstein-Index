...
### Subtitle
Why seven agents with one boss, and not one giant brain

### Body Paragraph 1
The most natural first instinct when building an AI-powered system is to try the simplest possible thing: give one model one giant prompt and ask it to do everything. "Here are 1,396 magazine issues. Here is the DOJ Epstein Library. Cross-reference every name, investigate every lead, and give me the confirmed connections." In theory, this is a perfectly reasonable request. In practice, it collapses almost immediately. The context window fills up. The model loses track of where it is. Names from page 400 start bleeding into assessments from page 800. There is no checkpoint, no audit trail, and no way to resume if something fails at hour six of an eight-hour run. The output is a black box: you get an answer, but you can't trace how any individual conclusion was reached.

### Body Paragraph 2
The alternative is to decompose the problem. Instead of one model doing everything, you assign specialized roles: one agent discovers issues, another downloads content, another extracts names, another cross-references them, another investigates the leads, and one sits above all of them making editorial decisions. Each agent has a narrow mandate, a defined input, a defined output, and its own queue of work. This is the multi-agent architecture, and the specific topology we chose is called hub-and-spoke.

### Body Paragraph 3
The metaphor is straightforward: imagine a newsroom. Reporters don't publish stories directly to the front page. They file copy. An editor reviews it, fact-checks it, decides whether it meets the publication's standards, and either runs it, kills it, or sends it back for revision. The reporters are specialists — one covers politics, another covers crime, another covers culture — but the editor is the single point of editorial control. No story reaches the public without the editor's sign-off.

### Body Paragraph 4
That is exactly how this system works. Miranda, the Editor agent, is the hub. The other six agents are spokes. Each spoke operates autonomously within its domain — the Scout discovers issues without waiting for the Courier, the Detective cross-references names without waiting for the Researcher — but every result flows back to Miranda. She is the only agent that can write a verdict to the database. She is the only agent that can confirm or reject a connection. When the Researcher produces a dossier saying "this person appears in Epstein's records," that dossier sits in Miranda's inbox until she reviews it. If she's not convinced, it doesn't get published.

### Body Paragraph 5
This was a deliberate architectural decision, not a default. There were other options. A pure pipeline chain — Scout feeds Courier feeds Reader feeds Detective feeds Researcher feeds Editor, strictly sequential — would be simpler to build but would mean the entire system runs at the speed of its slowest stage. If the Detective takes ten minutes on a difficult name, the Researcher sits idle, the Editor sits idle, and no work moves anywhere in the pipeline. A peer-to-peer mesh — where every agent can talk to every other agent, and they collectively decide what to do next — would be more "intelligent" in theory, but it introduces a coordination nightmare. If the Detective and the Researcher disagree about whether a lead is worth pursuing, who breaks the tie? If two agents try to write to the database at the same time, which write wins? Decentralized systems need consensus protocols, conflict resolution, and a way to handle contradictions. For a production system processing thousands of names, that complexity is a liability, not a feature.

### Body Paragraph 6
Hub-and-spoke avoids both problems. The agents run concurrently — when the Scout discovers a new issue, the Courier is already downloading the previous one, the Reader is extracting features from the one before that, and the Detective is cross-referencing names from even earlier. The pipeline is never idle. But all results converge on a single decision-maker, which eliminates the coordination problem entirely. There are no contradictory writes, no consensus votes, no conflict resolution. There is Miranda, and what she says goes.

### Body Paragraph 7
The communication mechanism is deliberately simple. Each agent has an inbox (a queue where Miranda places tasks) and an outbox (a queue where the agent places results). Miranda pushes a task — "cross-reference these twelve names" — into the Detective's inbox. The Detective works through them, pushes the results into his outbox. Miranda collects the results, validates them, writes the verdicts to the database, and then pushes new tasks — "investigate this confirmed lead" — into the Researcher's inbox. The message-passing is one-to-one. Agents never talk to each other directly. They only talk to Miranda, and she decides what happens next.

### Body Paragraph 8
This gives you three concrete engineering benefits. First, it prevents contradictory updates. If the Detective says "confirmed match" but the Researcher later determines the match was a different person with the same name, there is no race condition. Miranda sees both results, weighs them, and writes a single coherent verdict. Second, it ensures a single auditable trail. Every decision in the system — every confirmed connection, every rejection, every "needs more investigation" — traces back to Miranda's editorial review. You can reconstruct exactly why any conclusion was reached, in order, with timestamps. Third, it makes the system modular. If the Detective's methodology needs to change — say, adding a new evidence source beyond the Black Book and DOJ — you swap out the Detective's code without touching the Researcher, the Editor, or anyone else. The inbox/outbox interface stays the same. The rest of the system doesn't know or care that the Detective now checks three sources instead of two.

### Body Paragraph 9
It is worth being honest about what these agents are and what they are not. In the current landscape of AI discourse, the word "agent" carries enormous weight. It evokes images of autonomous systems that plan, reason, adapt, and pursue goals independently. The academic literature describes agent architectures with self-modifying code, emergent strategies, and recursive self-improvement. Some of that is real and some of it is aspirational. What we built here is closer to the practical end of the spectrum: these are task workers with editorial oversight. Each agent receives a specific, well-defined task from Miranda, executes it according to a set of instructions, and returns a structured result. The agent decides *how* to accomplish the task — the Detective decides which search strategy to use, the Researcher decides which graph queries to run — but *what* to work on and *whether the result is acceptable* are Miranda's calls.

### Body Paragraph 10
That distinction matters because it maps onto a real tension in AI systems design: autonomy versus control. A fully autonomous agent would not need Miranda. It would discover issues, extract names, cross-reference them, investigate leads, and publish verdicts entirely on its own. It would learn from its mistakes and refine its own methodology over time. In principle, it could even rewrite its own code when it finds a better approach. This is not science fiction — research prototypes exist that do exactly this. But for a system whose credibility depends on the accuracy of every individual verdict, autonomy is a risk. A single false positive — one person incorrectly labeled as an Epstein connection — would undermine the entire project. The editorial bottleneck is not a limitation. It is the feature.

### Body Paragraph 11
The newsroom analogy is not incidental. Real newsrooms operate this way for the same reason: the cost of publishing something wrong is catastrophic, so every piece of output passes through an editor who is personally accountable for it. Reporters have latitude in how they investigate — which sources they consult, which angles they pursue — but the editor controls what gets published and under what standard. In our system, Miranda reviews every single dossier. She has rejected dossiers that the Researcher rated as HIGH confidence, and she has confirmed connections that the Detective initially flagged as ambiguous. She is the human-in-the-loop, except she's not human — she's an AI agent running a more capable model (Claude Opus for quality-critical reviews, Claude Sonnet for routine routing), applying a consistent evidentiary standard across hundreds of cases without fatigue, without bias drift, and without the "it's Friday at 5pm" effect that degrades human editorial judgment over long runs.

### Body Paragraph 12
This architecture was built piece by piece, not designed in advance. The first version of the system was a single script that ran each stage sequentially. That worked for ten issues. It became untenable at fifty. The second version split the work into separate agents but let them all write to the database independently, and the first time two agents tried to update the same record simultaneously, the data corrupted. The third version introduced Miranda as the single writer — the hub — and the system stabilized. Each iteration was a response to a real failure, not an abstract architectural preference. That is worth noting because it reflects a general truth about building AI systems: you do not start with the elegant architecture. You start with the thing that works, you watch it break, and you redesign around the failure modes.

### Body Paragraph 13
Looking forward, the gap between what these agents do now and what autonomous agents will do in the near future is closing rapidly. Today, Miranda follows a fixed set of editorial rules: COINCIDENCE verdicts are auto-rejected, HIGH-confidence dossiers default to confirmed, and everything else gets a structured review. Tomorrow, she could learn her own editorial standards from the accumulating corpus of her own past decisions. Today, the Detective checks two sources — the Black Book and the DOJ library. Tomorrow, it could autonomously discover new evidence sources and integrate them without human intervention. Today, each agent's methodology is written by a human. Tomorrow, agents could propose, test, and deploy their own methodology improvements through a structured self-improvement loop — and in fact, the infrastructure for that already exists in this system (every 30 minutes, each agent proposes methodology changes in a structured WHAT/WHY/HOW format), though the proposals currently require human approval before deployment.

### Body Paragraph 14
But "tomorrow" is not today, and the honest assessment is that we are building sophisticated task workers, not autonomous intelligences. The agents are effective precisely because their scope is constrained. The Detective is excellent at cross-referencing names because that is all it does, thousands of times, with consistent methodology and complete focus. If you asked it to also extract features from PDFs, it would need an entirely different set of instructions, a different model configuration, and different error-handling logic. Specialization is the point. The system's intelligence is not in any individual agent — it is in the architecture that connects them, the message-passing that coordinates them, and the editorial control that quality-gates their output. The whole is smarter than any of its parts, and that is exactly the design intent.

### Summary
Miranda reviews every dossier personally — no confirmed connection reaches the public index without editorial sign-off. The other six agents work concurrently, each processing its own queue. The hub-and-spoke topology was not an upfront design decision but an emergent response to real failures: sequential processing was too slow, independent database writes caused corruption, and decentralized coordination introduced conflicts that no amount of clever protocol could reliably resolve. The editorial bottleneck is the feature, not the limitation.
